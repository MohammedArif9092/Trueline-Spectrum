...
export default async function Page({ params, searchParams }: Props) {
  ...
  return (
    <div>
      <AdminPageHeader title="Edit Organization" description={item.name}
        action={<form action={deleteOrganization}><input type="hidden" name="id" value={item.id} />
          <ActionButton label="Delete" icon="trash" className="btn-outline" confirm="Delete this organization?" /></form>} />
      ...
      <OrganizationForm item={item} action={updateOrganization.bind(null, id)} />
    </div>
  );
}
